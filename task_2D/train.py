...
def train_net(net,                       
    ...
    scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer,T_0=5,T_mult=2,eta_min=0.00001,last_epoch=-1)
        ...
        if dice/len(val_loader) > best_dice:
            ...
            if save_cp:
                try:
                    ...
                    pass
                torch.save(net.state_dict(), dir_checkpoint + f'/top_t_cbam_UNet2.pth')
...
if __name__ == '__main__':
    ...
    net = top_t_cbam_UNet(1,1,percent_t=0.3).to(device=device)
    if torch.cuda.device_count() > 1:
        net = nn.DataParallel(net,device_ids=[0,1,2,4]) 
