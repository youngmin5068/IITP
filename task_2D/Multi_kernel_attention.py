...
class AAU_spatial(nn.Module):
    def __init__(self,dim):
        ...
        self.conv3 = nn.Conv2d(dim,1,kernel_size=1)
        self.sigmoid = nn.Sigmoid()

        self.conv4 = nn.Conv2d(dim,dim,kernel_size=1)
        self.bn3 = nn.BatchNorm2d(dim)
        
        self.resample = nn.Conv2d(1,dim,kernel_size=1)
...
class CA(nn.Module):
    def __init__(self,dim):
        ...
        self.avgpool_x = nn.AdaptiveAvgPool2d((1, 256))  # X 축에 대한 pooling
        self.avgpool_y = nn.AdaptiveAvgPool2d((256, 1))  # Y 축에 대한 pooling
    ...
    def forward(self, x):
        ...
        split_1, split_2 = torch.split(combined, split_size_or_sections=256, dim=2) 
