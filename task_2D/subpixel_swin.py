...
class OutS2D(nn.Module):
    def __init__(self,in_channels,mid_channels,out_channels):
        super(OutS2D,self).__init__()
        self.double_conv3_3 = nn.Sequential(
            nn.Conv2d(in_channels, mid_channels,kernel_size=3,padding=1,bias=False),
            nn.InstanceNorm2d(mid_channels),
            nn.PReLU(),
...
class SubPixelNetwork(nn.Module):
    def __init__(self):
        ...
        self.s2d = Space2Depth(8,4,4) # output shape: 4x1024x1024 -> 16x512x512
    ...
    def forward(self,x):
        ...
        x = self.d2s(x) # output shape: 4x1024x1024 -> 8x1024x1024
        x = self.s2d(x) # output shape: 16x512x512 -> 16x512x512
        x = self.double_conv2(x) # 1x512x512
...
class subpixel_SwinUNetr(nn.Module):
    """
    ...
    def forward(self, x_in):
        sp_x = self.SPNet(x_in)
        hidden_states_out = self.swinViT(sp_x, self.normalize)
        enc0 = self.encoder1(sp_x)
...
# model = subpixel_SwinUNetr(spatial_dims=2,img_size=(512,512),in_channels=1,out_channels=1)
